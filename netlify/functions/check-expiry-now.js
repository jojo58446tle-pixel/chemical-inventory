...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (!lots.length) return json(200, { ok:true, sent:false, count:0 });
    ...
    const pending = lots.map(lot => ({ lot, level:getAlertLevel_(today, parseDate_(lot.expiry_date)) }))
      .filter(x => x.level && !sentKeys.has(`${x.lot.id}|${x.level}`));
    if (!pending.length) return json(200, { ok:true, sent:false, count:0 });
    ...
    for (const [level, groupLots] of groups) {
      ...
      const dingResponse = await fetch(process.env.DINGTALK_WEBHOOK_URL, {
        method:"POST",
        headers:{ "Content-Type":"text/plain; charset=utf-8" },
        body:message
      });
      const responseText = await dingResponse.text();
      if (!dingResponse.ok) throw new Error(`DingTalk HTTP ${dingResponse.status}: ${responseText}`);
