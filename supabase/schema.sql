...
begin
  if current_setting('request.jwt.claim.role', true) <> 'authenticated' then raise exception 'Unauthorized'; end if;
  insert into chemical_lots(material_id,lot_no,received_qty,remaining_qty,received_date,expiry_date,created_by)
  values(p_material_id,p_lot_no,p_qty,p_qty,p_received_date,p_expiry_date,null)
