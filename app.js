...
function statusMeta(status){
  ...
  if(status==="NEAR_EXPIRY")return {label:"NEAR EXPIRY",thai:"ใกล้หมดอายุมาก — ควรแจ้งตรวจสอบ",cls:"warn",icon:"!"};
...
async function testChemicalDingTalk(mode="workflow_text"){
  const response=await fetch("/.netlify/functions/test-dingtalk-chemical",{method:"POST",headers:{Authorization:`Bearer ${adminToken}`,"Content-Type":"application/json"},body:JSON.stringify({source:"CHEMICAL_ALERT_PAGE",mode})});
  const result=await response.json();
...
async function alerts(){
  ...
  $("#page").innerHTML=`<div class="dashboard-columns"><section class="panel"><div class="panel-head"><div><span class="panel-icon">□</span><b>General Material Expiry Check</b></div><div class="chemical-alert-actions"><span class="chip">LOOKUP ONLY</span><button id="testGeneralWorkflow" class="secondary small-btn">ทดสอบ Workflow 2</button></div></div><p class="helper">General Material ไม่มี Stock Tracking ในระบบนี้ จึงตรวจจาก Material Code + MFG Date ณ จุดใช้งาน และแจ้ง DingTalk แบบ Manual เมื่อพบความเสี่ยง</p><div class="verify-search"><input id="alertCode" placeholder="Material Code"><input id="alertMfg" type="date"><button id="alertCheck" class="primary">Check</button></div><div id="alertGeneralResult" class="result-space"><div class="empty-state">กรอก Material Code และ MFG Date</div></div></section><section class="panel"><div class="panel-head"><div><span class="panel-icon">⚗</span><b>Chemical Automatic Expiry Alert</b></div><div class="chemical-alert-actions"><button id="testWorkflowText" class="secondary small-btn">ทดสอบ DingTalk</button><button id="send" class="primary small-btn">ส่งแจ้งเตือนจริง</button></div></div><div id="chemicalDingStatus" class="notice info"><b>Chemical DingTalk Workflow</b><span>Test จะส่งข้อความ Text พร้อม Keyword “แจ้งเตือนสารเคมี” ตาม Workflow เดิม</span></div><div class="list">${lots.length?lots.slice(0,30).map(lotCard).join(""):'<div class="empty-state">ไม่มีรายการเข้าเงื่อนไข</div>'}</div>${lots.length>30?`<div class="helper">แสดง 30 จาก ${fmt(lots.length)} รายการ</div>`:""}</section></div>`;

  const runChemicalDingTest=async(mode,buttonId,label)=>{const btn=$(buttonId),box=$("#chemicalDingStatus");try{btn.disabled=true;btn.textContent="กำลังทดสอบ...";const j=await testChemicalDingTalk(mode);box.className="notice info";box.innerHTML=`<b>Webhook Accepted — ${esc(label)}</b><span>HTTP ${esc(j.http_status)} • ${esc(j.sent_at||"")}</span><small>Response: ${esc(j.response||"(empty response)")}</small><small>${j.robot_ack?"Robot ACK = errcode 0":"ยังไม่ยืนยันว่า DingTalk ส่งข้อความออกจริง"}</small>`;alert(`Webhook รับ Request แล้ว (${label}) — กรุณาดู DingTalk ว่ามีข้อความเด้งหรือไม่`);}catch(e){box.className="notice danger";box.innerHTML=`<b>DingTalk Test = FAIL</b><span>${esc(e.message)}</span>`;alert(`ทดสอบไม่สำเร็จ: ${e.message}`);}finally{btn.disabled=false;btn.textContent=label;}};
  $("#testWorkflowText").onclick=()=>runChemicalDingTest("workflow_text","#testWorkflowText","ทดสอบ DingTalk");
  ...
  $("#testGeneralWorkflow").onclick=async()=>{
    const btn=$("#testGeneralWorkflow");
    try{
      btn.disabled=true;btn.textContent="กำลังทดสอบ...";
      const j=await notifyGeneralMaterialDingTalk({test:true,source:"GENERAL_EXPIRY_ALERT_PAGE"});
      alert(`ส่ง Test ไป Workflow 2 แล้ว — กรุณาดู DingTalk\n${j.sent_at?new Date(j.sent_at).toLocaleString("th-TH"):""}`);
    }catch(e){alert(`Workflow 2 ยังไม่พร้อม: ${e.message}`);}finally{btn.disabled=false;btn.textContent="ทดสอบ Workflow 2";}
  };

  $("#alertCheck").onclick=async()=>{
    const code=normalizeCode($("#alertCode").value),mfg=$("#alertMfg").value;
    if(!code||!mfg)return alert("กรอก Material Code และ MFG Date");
    try{
      const r=await mslLookupCode(code);
      if(!r?.found){$("#alertGeneralResult").innerHTML=`<div class="notice danger"><b>MATERIAL_CODE_NOT_FOUND</b><span>${esc(code)} ไม่มี Mapping — ระบบจะไม่เดา Material Group</span></div>`;return;}
      if(r.profile_verification_required){$("#alertGeneralResult").innerHTML=`<div class="notice warning"><b>PROFILE VERIFICATION REQUIRED</b><span>พบหลาย Storage Profiles — ระบบจะไม่เลือกแทน กรุณายืนยัน Profile ที่ถูกต้องก่อนคำนวณ Expiry</span></div><div class="profile-grid">${allProfilesHtml(r.storage_profiles)}</div>`;return;}
      const profile=r.selected_profile;
      const c=calcGeneralExpiry(mfg,profile);
      if(!c||c.error){$("#alertGeneralResult").innerHTML=c?.error==="MFG_DATE_IN_FUTURE"?'<div class="notice danger"><b>MFG DATE INVALID</b><span>MFG Date ต้องไม่เป็นวันที่ในอนาคต</span></div>':'<div class="notice danger">ไม่สามารถคำนวณ Expiry ได้</div>';return;}
      const m=statusMeta(c.status),notifyAllowed=["EXPIRING_SOON","NEAR_EXPIRY","EXPIRED"].includes(c.status);
      const payload={material_code:r.material_code,material_group:r.material_group,mfg_date:mfg,expiry_date:c.expiry,remaining_days:c.remainingDays,remaining_percent:c.remainingPercent,status:c.status,profile_no:profile?.profile_no||null,shelf_life:profile?.shelf_life_text||null,storage_temperature:profile?.storage_temperature||null,storage_humidity:profile?.storage_humidity||null,packaging:profile?.packaging||null};
      $("#alertGeneralResult").innerHTML=`<div class="detail-grid compact"><div><span>Material Code</span><b>${esc(r.material_code)}</b></div><div><span>Material Group</span><b>${esc(r.material_group)}</b></div><div><span>Shelf Life</span><b>${esc(profile?.shelf_life_text||"—")}</b></div><div><span>Storage</span><b>${esc([profile?.storage_temperature,profile?.storage_humidity].filter(Boolean).join(" • ")||"—")}</b></div></div><div class="expiry-result ${m.cls}"><b>${m.label}</b><span>Expiry ${esc(c.expiry)} • Remaining ${fmt(c.remainingDays)} Days • ${c.remainingPercent}%</span><small>${esc(m.thai)}</small></div>${notifyAllowed?`<div class="ding-alert-box ${m.cls}"><div><b>${c.status==="EXPIRED"?"Expired Material — HOLD":"Manual DingTalk Alert"}</b><span>General Material ไม่มี Stock Tracking จึงแจ้งจากผลตรวจหน้างานเท่านั้น</span></div><textarea id="generalAlertRemark" rows="3" placeholder="Warehouse / IQC Remark, Location, Lot หรือเหตุผลที่แจ้ง"></textarea><button id="generalNotify" class="${c.status==="EXPIRED"?"danger":"ding-btn"}">${c.status==="EXPIRED"?"แจ้ง DingTalk & HOLD":"แจ้งเตือน DingTalk"}</button><small id="generalNotifyState">Workflow 2 • Keyword: แจ้งเตือนวัสดุทั่วไป</small></div>`:`<div class="notice info"><b>ยังไม่เข้าเงื่อนไขแจ้งเตือน</b><span>Status = VALID จึงแสดงผลตรวจสอบอย่างเดียว</span></div>`}`;
      const nb=$("#generalNotify");
      if(nb){nb.onclick=async()=>{const remark=$("#generalAlertRemark")?.value.trim()||"";if(!remark)return alert("กรุณาระบุ Remark / จุดที่พบก่อนส่ง DingTalk");if(!confirm(`ยืนยันส่ง General Material Alert สำหรับ ${payload.material_code} (${payload.status}) ?`))return;nb.disabled=true;nb.textContent="กำลังส่ง...";const state=$("#generalNotifyState");try{const j=await notifyGeneralMaterialDingTalk({...payload,remark,source:"GENERAL_EXPIRY_ALERT_PAGE"});nb.textContent="ส่ง DingTalk แล้ว ✓";nb.className="success";if(state)state.textContent=`ส่งสำเร็จ ${new Date(j.sent_at||Date.now()).toLocaleString("th-TH")}`;}catch(e){nb.disabled=false;nb.textContent=payload.status==="EXPIRED"?"แจ้ง DingTalk & HOLD":"แจ้งเตือน DingTalk";if(state)state.textContent=`ส่งไม่สำเร็จ: ${e.message}`;alert(`ส่ง DingTalk ไม่สำเร็จ: ${e.message}`);}};}
    }catch(e){$("#alertGeneralResult").innerHTML=`<div class="notice danger"><b>CHECK FAILED</b><span>${esc(e.message)}</span></div>`;}
  };
}
    ...
    try{
      ...
      if(mfg&&!r.profile_verification_required){
        if(calc?.error){expiryBlock='<div class="notice danger"><b>MFG DATE INVALID</b><span>MFG Date ต้องไม่เป็นวันที่ในอนาคต</span></div>';}
          ...
          const notifyAllowed=["EXPIRING_SOON","NEAR_EXPIRY","EXPIRED"].includes(calc.status);
          if(notifyAllowed){
            notifyBlock=`<div class="ding-alert-box ${meta.cls}"><div><b>แจ้งเตือนหน้างานไปยัง DingTalk</b><span>ใช้เมื่อ Warehouse พบวัสดุใกล้หมดอายุหรือหมดอายุและต้องการ Quality/IQC ตรวจสอบ</span></div><textarea id="whAlertRemark" rows="3" placeholder="Warehouse Remark / จุดที่พบ / Lot หรือ Location (ถ้ามี)"></textarea><button id="whNotifyDing" class="${calc.status==='EXPIRED'?'danger':'ding-btn'}">${calc.status==='EXPIRED'?'แจ้ง DingTalk & HOLD':'แจ้งเตือน DingTalk'}</button><small id="whNotifyState">Workflow 2 • Keyword: แจ้งเตือนวัสดุทั่วไป</small></div>`;
            ...
            notifyBlock='<div class="notice info"><b>ยังไม่เข้าเงื่อนไขแจ้งเตือน</b><span>Status = VALID จึงไม่เปิดปุ่ม DingTalk</span></div>';
          }
        }
      }
