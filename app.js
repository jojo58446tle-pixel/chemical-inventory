...
async function receive(){
  ...
  $("#save").onclick=async()=>{try{const code=$("#code").value.trim(),supplier=$("#supplier").value.trim(),lot=$("#lot").value.trim(),qty=+$("#qty").value,unit=$("#unit").value.trim(),product=$("#productName").value.trim(),brand=$("#brand").value.trim(),pack=$("#packageSize").value.trim(),rd=$("#rdate").value,q=$("#mfgQr").value,l=$("#mfgLabel").value,mfg=chosenMfg(),ex=$("#exp").value,location=$("#location").value.trim();if(!code||!supplier||!lot||!(qty>0)||!unit||!product||!brand||!pack||!rd||!q||!mfg||!ex||!location)return alert("กรอกข้อมูลให้ครบ (Product Name และ Brand ใส่ NA ได้)");if(q&&l&&q!==l&&!$("input[name=mfgSource]:checked"))return alert("วันที่ผลิตไม่ตรงกัน กรุณาเลือกวันที่ที่จะใช้");await saveMaster(false);const {error}=await sb.rpc("receive_stock_v1",{p_material_id:mat.id,p_supplier_code:supplier,p_lot_no:lot,p_qty:qty,p_unit:unit,p_product_name:product,p_brand:brand,p_package_size:pack,p_received_date:rd,p_mfg_qr_date:q,p_mfg_label_date:l||null,p_mfg_used_date:mfg,p_mfg_source:q&&l&&q!==l?$("input[name=mfgSource]:checked").value:"QR",p_expiry_date:ex,p_location:location});if(error)throw error;alert("บันทึกรับเข้าสารเคมีแล้ว");render("dashboard");}catch(e){console.error(e);alert(`บันทึกไม่สำเร็จ: ${e.message}`);}};
}
...
async function editStockLot(l){
  ...
  $("#page").innerHTML=`<div class="form">
    ...
    <label>Material Name<input value="${esc(l.materials?.material_name||"")}" disabled></label>
    ...
    <label>Expiry Date (วันหมดอายุ)<input id="eExpiry" type="date" value="${esc(l.expiry_date||"")}"></label>
  ...
  $("#eSave").onclick=async()=>{try{const code=$("#eCode").value.trim(),reason=$("#eReason").value.trim();if(!reason)return alert("กรุณาระบุเหตุผลที่แก้ไข");const {data:mat,error:matError}=await sb.from("materials").select("id").eq("material_code",code).maybeSingle();if(matError)throw matError;if(!mat)return alert("ไม่พบ Material Code นี้ใน Material Master");const q=$("#eMfgQr").value,label=$("#eMfgLabel").value,used=label||q,source=label&&q&&label!==q?"LABEL":"QR";const {error}=await sb.rpc("update_chemical_lot_v1",{p_lot_id:l.id,p_material_id:mat.id,p_supplier_code:$("#eSupplier").value.trim(),p_lot_no:$("#eLot").value.trim(),p_received_qty:+$("#eQty").value,p_unit:$("#eUnit").value.trim(),p_product_name:$("#eProduct").value.trim()||"NA",p_brand:$("#eBrand").value.trim()||"NA",p_package_size:$("#ePack").value.trim(),p_received_date:$("#eReceive").value,p_mfg_qr_date:q||null,p_mfg_label_date:label||null,p_mfg_used_date:used||null,p_mfg_source:source,p_expiry_date:$("#eExpiry").value,p_location:$("#eLocation").value.trim(),p_reason:reason});if(error)throw error;alert("แก้ไขรายการแล้ว");render("stock");}catch(e){console.error(e);alert("แก้ไขไม่สำเร็จ: "+e.message);}};
}
